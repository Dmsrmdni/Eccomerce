            ...
            <div class="row gutter-40">
                            ...
                            <div class="flexslider">
                                <div class="slider-wrap" data-lightbox="gallery">
                                    ...
                                    <div class="slide" data-thumb="{{asset($image->gambar_produk)}}"><a
                                            ...
                                            data-lightbox="gallery-item"><img src="{{asset($image->gambar_produk)}}"
                                                alt="{{ $produks->name }}"></a>
                                    </div>
                                    @endforeach
                                </div>
                ...
                <div class="col-md-5 product-desc">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        @else
                        <div class="product-price text-dark">Rp. {{ number_format($produks->harga, 0, ',', '.') }}</div>
                        @endif
                        <!-- Product Single - Price End -->
                        ...
                        @endphp
                        <div class="product-rating">
                            @for($i = 1; $i <= $total; $i++) <i class="icon-star3 icon-lg" style="color: yellow">
                                </i>
                                @endfor
                                @for ($j = $total+1; $j <= 5; $j++) <i class="icon-star3"></i>
                                    @endfor
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-2">
                            <span>terjual : {{ $produks_terjual }} </span>
                        </div>
                        <div class="col-lg-2">
                            <span>ulasan : {{ $produks->reviewProduk->count() }}</span>
                        </div>
                    </div>
                    ...
                    <ul class="iconlist">
                        ...
                        <li><i class="icon-caret-right"></i>kategori : <a
                                href="/produk?kategori={{ $produks->kategori_id }}" class="mx-1">{{
                                $produks->kategori->name }}</a> ,
                    ...
                    <form id="tambahKeranjang" action="{{ route('keranjang.store') }}" method="POST">
                        ...
                        <div role="group">
                            ...
                            <label for="{{ $ukuranProduk->id }}"
                                class="btn btn-sm btn-outline-secondary fw-normal ls0 nott">{{
                                $ukuranProduk->ukuran->ukuran }}</label>
                            @endforeach
                        </div>
                        ...
                        <div class="quantity clearfix">
                            <input type="button" value="-" class="minus">
                            <input type="number" step="1" min="1" max="{{ $produks->stok }}" name="jumlah" value="1"
                                title="Qty" class="qty" />
                            <input type="button" value="+" class="plus">
                        </div>
                        @auth
                        <a onclick="event.preventDefault(); document.getElementById('tambahKeranjang').submit();"
                            class="add-to-cart button m-0 float-end">Add to cart</a>
                        @endauth
                        @guest()
                        <a id="auth" class="add-to-cart button m-0 float-end">Add to cart</a>
                        @endguest
                    </form>
                </div>
                                    ...
                                    <div class="col ps-12">
                                        ...
                                        <div class="entry-meta no-separator">
                                            <ul>
                                                ...
                                                $harga = $produk->harga - $diskon;
                                                @endphp
                                                <li class="text-secondary"><del>Rp. {{number_format($produk->harga,
                                                        0,',', '.') }} </del> <ins class="text-dark"> Rp. {{
                                                ...
                                                <li>
                                                    <div class="product-rating">
                                                        @for($i = 1; $i <= $total; $i++) <i class="icon-star3"
                                                            style="color: yellow"></i>
                                                            @endfor
                                                            @for ($j = $total+1; $j <= 5; $j++) <i class="icon-star3">
                                                                </i>
                                                                @endfor
                                                    </div>
                                                </li>
                ...
                <div class="col-12 mt-5">
                    ...
                    <div class="tabs tabs-bb clearfix">
                        ...
                        <ul class="tab-nav clearfix">
                            <li><a href="#all">All ({{ $allReview_produks->count() }})</a></li>
                                    ...
                                    <ol class="commentlist clearfix my-3">
                                        ...
                                        <li class="comment even thread-even depth-1" id="li-comment-1">
                                            <div id="comment-1" class="comment-wrap clearfix">
                                                ...
                                                <div class="comment-content clearfix">
                                                    <div class="comment-author">{{ $review_produk->user->name
                                                        }}<span>{{ $review_produk->created_at }}</span></div>
                                                    ...
                                                    @endphp
                                                    <div class="review-comment-ratings">
                                                        @for($i = 1; $i <= $totalRating; $i++) <i
                                                            class="icon-star3 icon-lg" style="color: yellow">
                                                            </i>
                                                            @endfor
                                                            @for ($j = $totalRating+1; $j <= 5; $j++) <i
                                                                class="icon-star3 icon-lg">
                                                                </i>
                                                                @endfor
                                                    </div>
                                                </div>

                ...
                <tbody>
                    ...
                    <tr class="cart_item">
                        ...
                        <td class="cart-product-price">
                            <span class="amount">Rp. {{ number_format($wishlist->produk->harga, 0, ',', '.') }}</span>
                        </td>
                        <td class="cart-product-price">
                            <span class="amount">{{ number_format($wishlist->produk->diskon, 0, ',', '.') }}%</span>
                        </td>
                        <td class="cart-product-remove">
                            <form id="delete{{ $wishlist->id }}" action="{{ route('wishlist.destroy', $wishlist->id) }}"
                                method="POST">
                                @csrf
                                @method('delete')
                                <a onclick="event.preventDefault(); document.getElementById('delete{{ $wishlist->id }}').submit();"
                                    class="remove" title="Hapus Produk ini"><i class="icon-line-delete icon-lg"></i></a>
                            </form>
                        </td>
                    </tr>
                    @endforeach
                    ...
                    <tr>
                        <td colspan="5" class="text-center">
                            <div class="my-3">
                                <img src="{{ asset('images/no_review.png') }}" width="80px" alt="" srcset="">
            ...
            <div class="col-lg-auto pe-lg-0">
                <form id="deleteAll" action="/deleteAllWishlist">
                    ...
                    <a id="btnDeleteAll" class="button button-3d button-black m-0 mt-2 mt-sm-0 me-0">Hapus all
                        produk</a>
                    @else
                    <a id="wishlist" class="button button-3d button-black m-0 mt-2 mt-sm-0 me-0">Hapus all Wishlist</a>
                    @endif
                </form>
            </div>
        </div>
